...
export default (state = reducerDefaultState, action) => {
	...
	switch (action.type) {
		...
		case 'EXECUTE_OPERATION':
			return { input: 0, memory: evaluate(target, state.memory, state.operation) || state.input, operation: action.operation, displayMemory: true }; //display Memory == true? evalute against memory, not against display
